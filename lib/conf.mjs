...
export class Backend {
  ...
  static localRe = "(?<local>\\.?\\/[^/\\r\\n]*(?:\\/[^/\\r\\n]+)*)"
  static finalRe = `${this.modeRe}(?<raw>${this.urlRe}|${this.localRe})`
  static re = new RegExp(this.finalRe)
  constructor(conf, rcbase, name, p) {
    this.conf = conf
    this.name = name
    const m = Backend.re.exec(p).groups
    this.path = m.raw
    this.mode = m.mode || defaults.mode
    this.remote = !m.local
    this.proto = m.proto
    this.protos = []
    this.user = m.user
  ...
  syncpath(target) {
    return path.join(this.path, this.conf.rc.host, target.name)
  }
  sshline() {
  ...
  sshhost(target) {
    const tail = path.join(this.tail || "/", this.conf.rc.host, target.name)
...
export const mkconf = (...args)=> {
  ...
  Object.entries(conf.rc.backends).forEach(be => {
    const beo = new Backend(conf, conf.rc.rcbase, be[0], be[1])
