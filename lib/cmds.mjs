...
export const mkcmds =(conf)=> {
    ...
    for (let tn = 0; tn < tnames.length; tn++) {
      ...
      for (let rn = 0; rn < rnames.length; rn++) {
        ...
        if (repo.mode == "snap") {
          ...
          if (plat == "win32") {
            await conf.util.robo(repo, target, ...path)
          } else {
    ...
    if (!tname) {
      ...
      if (be.mode == "snap") {
        ...
        if (plat == "win32") {
          ...
          await conf.util.roborestore(be, path, ...paths)
        } else {
          if (be.proto == "ssh:") {
  ...
  conf.cmds.make("clean", {
    sig: "[REPO]",
    info: "Clean REPO, or all repos.",
  }, async (conf, rname, ...args) => {
    try {
      await conf.util.eachrepo(rname, "cache", "--cleanup", ...args)
    } catch (e) {
      if (e instanceof BackendError) {
        conf.logger.err("Invalid backend given. This command only works with repos.", { indent: 1 })
        conf.cmds.help("clean")
        process.exit(1)
      } else throw e
    }
  })
