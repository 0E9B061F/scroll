...
export const mkcmds =(conf)=> {
  ...
  conf.cmds.make("backup:b", {
    ...
    for (let tn = 0; tn < tnames.length; tn++) {
      ...
      const path = conf.registry.targets[target].path
      const exclude = conf.registry.targets[target].exclude.map(e => `--exclude=${e}`)
      const opts = ["--no-scan"]
      ...
      for (let rn = 0; rn < rnames.length; rn++) {
        ...
        if (repo.mode == "snap") {
          ...
          await conf.util.sync(repo, ...path)
        }
  ...
  conf.cmds.make("restore:r", {
    ...
    if (!tname) {
      ...
      conf.logger.err("must specify a aingle REPO to restore from", {indent: 1})
      conf.cmds.help("restore")
    } else if (!conf.registry.snap[rname]) {
      conf.logger.err("backend doesn't exist or isn't a repo", {indent: 1})
      conf.cmds.help("restore")
    } else if (!path) {
      ...
      const tags = conf.util.consume(tname)
      const repo = conf.registry.snap[rname]
      await conf.util.snap(repo, "restore", `--host=${conf.rc.host}`, `--target=${path}`, ...tags, "latest", ...args)
    }
  })
