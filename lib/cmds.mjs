  ...
  conf.cmds.make("backup:b", {
    ...
    for (let tn = 0; tn < tnames.length; tn++) {
      ...
      for (let rn = 0; rn < rnames.length; rn++) {
        repo = rnames[rn]
        await conf.util.perform(repo, "backup", ...opts, ...tags, ...path, ...exclude)
      }
    }
  })
  ...
  conf.cmds.make("report", {
    ...
    if (!conf.mailer) {
      ...
      if (long) {
        ...
        await conf.util.email(`[${os.hostname()} scroll] (0:OK) Weekly Report`, 
      ...
      await conf.util.email(`[${os.hostname()} scroll] (0:OK) Daily Report`, 
